...
interface Props {
  ...
  category?: MonetizationCategory | string;
  tags?: string[];
  limit?: number;
  page?: string;
  title?: string;
...
export function HealthProductRecommendations({ condition, category, tags = [], limit = 4, page = "/", title }: Props) {
  ...
  if (category) {
    const byCat = getAffiliateByCategory(category as string, limit);
    if (byCat.length) products = byCat;
    else products = products.filter((p) => p.category.toLowerCase() === (category as string).toLowerCase() || p.category === category);
  }
